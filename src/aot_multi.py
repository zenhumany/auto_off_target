...
class Engine:
    ...
    def __init__(self, logname):
        ...
        #add by zenhumany at 2023.11.22
        self.dbops = None
        self.import_db = False
        self.db_handle = None
    # -------------------------------------------------------------------------

    def init_eng(self, args, db_frontend):
        ...
        #add by zenhumany at 2023.11.19
        self.arg_metadata = args.arg_metadata
        #end add
        ...
        self.deps = Deps(args)
        #modified by zenhumany at 2023.11.22
        if not self.dbops:
            self.dbops = AotDbOps(
                self.db_handle, self.bassconnector, self.deps, args)
        self.cutoff = CutOff(self.dbops, args, self.bassconnector, self.deps)
        #add by lzh at 2023.07.31
        #pdb.set_trace( )
        if self.args.external_funcs_file:
            self.cutoff._init_external_funcs_whitelist( self.args.external_funcs_file )
        #end add
        ...
        if self.import_json and not self.import_db:
            self.import_db = True
            self.dbops.import_aot_db(args.import_json, args.lib_funcs_file,
                                     ...
                                     args.rdm_file,args.arg_metadata)
            #modified by zenhumany at 2023.11.22
            #if not self.dbops:
            self.dbops.create_indices()
        
        #add by zenhumany at 2023.12.14
        if args.smart_init and args.init_file:
            self.dbops.import_init_file( args.init_file )
        
        if args.smart_init and args.arg_metadata:
            self.dbops.load_arg_metadata( args.arg_metadata )
        #end add   
        ...
        #modified by zenhumany at 2021.12.14
        if self.rdm_file:
    ...
    def generate_off_target(self, function_names, depth=0):
        ...
        #add by zenhumany at 2024.04.15
        #fixed the bug functions have the same name in stub_files.
        #this is an agly soulation.
        stub_funcs_list = []
        #end add 
        ...
        for func in self.cutoff.external_funcs:
            ...
            fid = function["fid"]
            #add by zenhumany at 2024.04.15
            #if the stub have the same name with the previous stub function. not add this function into stub_files.
            name = function["name"]
            if name not in stub_funcs_list:
                stub_funcs_list.append(name)
            else:
                print("[WARNING]:ignore the function:%s may lead error.Need to prove this code is right." % name )
                continue
            #end add 
        ...
        for t_id in self.otgen.all_types:
            ...
            for t_id2 in self.otgen.all_types:
                ...
                if cl == "enum":
                    ...
                    if name == name2:
                        match = True
                        #modified by zenhumany at 2023.08.18
                        #In the following situaton, wo don't think t1 and t2 clashes.
                        #Because in the C language, it is legal for the following two definitions to exist at the same time.
                        #t1 type:
                        #    typedef struct pdu_data pdu_data_t;
                        #t2 type:
                        #    typedef struct pdu_data { 
                        #        comm_buffer_t *p_comm_buffer;
                        #        uint16_t size;  
                        #        uint16_t length;    
                        #        uint16_t window_start_inde  x;  
                        #        uint16_t valid_data_index;    
                        #        pdu_data_t *p_next;    
                        #        pdu_data_t *p_last;
                        #     } pdu_data_t;
                        #
                        t_def = t["def"]
                        t2_def = t2["def"]
                        if t_def.__contains__("{") and not t2_def.__contains__("{") or not t_def.__contains__("{") and t2_def.__contains__("{"):
                            #pdb.set_trace( )
                            match = False
                        #modified by zenhumany at 2023.11.24
                        #In the following situaton, wo don't think t1 and t2 clashes.
                        #14826 typedef struct sm_iface_attrs_t
                        #14827 {
                        #14828     bool is_msg_basic_type;
                        #14829     bool has_modified_listener;
                        #14830     bool has_updated_listener;
                        #14831     bool has_req_arrive_listener;
                        #14832     bool has_resp_arrive_listener;
                        #14833     bool force_enqueue;
                        #14834     bool has_race_condition;
                        #14835     uint8_t target_core;
                        #14836 } sm_iface_attrs_t;
                        #When const sm_iface_attrs_t and sm_iface_attrs_t are used at the same time,
                        #judging from the implementation of the _get_type_defs function in codegen, 
                        #the two typedefs will be merged and defined, which will not cause conflicts.
                        if "location" in t and "location" in t2:
                            #When the locations are the same, it means that the same typedef uses different qualifiers, which is not a conflict.
                            if t["location"] == t2["location"]:
                                match = False
                if match:
                    ...
                    if cl2 == "typedef":
                        ...
                        if t_id in self.deps.dup_types and t2["refs"][0] in self.deps.dup_types[t_id]:
                            continue
                    #add by zenhumany at 2024.04.15
                    t_id_dst = self.dbops._get_typedef_dst(t)  
                    t_id2_dst = self.dbops._get_typedef_dst(t2)
                    if t_id_dst == t_id2_dst:
                        continue
                    #end add
        ...
        for f_id in self.otgen.all_funcs:
            ...
            for f_id2 in self.otgen.all_funcs:
                ...
                if name == name2:
                    #modified by zenhumany at 2024.04.15
                    #if the f_id2, f_id in the stub_files at the same time.it will cause the redefinition functions error.
                    for each_key in stub_files.keys( ):
                        stub_file = stub_files[each_key]
                        if f_id in stub_file.funcs:
                            stub_file.funcs = stub_file.funcs - {f_id}
                            if f_id in self.cutoff.external_funcs:
                                 self.cutoff.external_funcs = self.cutoff.external_funcs - {f_id}
                                 print("[WARNING]: remove f_id=%d from self.cutoff.external_funcs" % f_id )
                            print("[WARNING]: remove f_id=%d from stub_files" % f_id )
                    continue
                    #end modified
                    # we've found a name clash right here
...
def main():
    ...
    parser.add_argument("--functions-file", default=None,
                        help='A file that needs to generate the off target fuzz driver function name')
    ...
    #add by zenhumany at 2023.12.14
    parser.add_argument("--arg-metadata", default=None,
                        help='Argument metadata json file obtained from Argument Value-Set Inference module')
    parser.add_argument("--smart-init", action='store_true',
                        help="Perform smart init based on the meta-information generated by the argument value set/type/deps inference module.")
    #end add
    #add by zenhumany at 2024.04.01
    parser.add_argument("--smart-init-path", default=None,
                        help="smart init metadata path.")
    #end add
    ...
    #add by lzh at 2023.07.31
    parser.add_argument("--external-funcs-file", default=None,help='A collection of user-specified external functions')
    #end add
    ...
    try:
        ...
        #if False == engine.init_eng(args, db_frontend):
        ...
        if args.functions_file:
           ...
           for each_line in lines:
               ...
               #add by zenhumany at 2023.12.14
               if args.smart_init:
                   arg_metadata = args.smart_init_path + "/" + each_line + "/metadata/arg_metadata.json"
                   if os.path.exists( arg_metadata):
                       args.arg_metadata = arg_metadata
                   init_file = args.smart_init_path + "/"  + each_line + "/metadata/" + each_line + "_aot_initdata.json"
                   if os.path.exists(init_file):
                       args.init_file = init_file 
               bRet = engine.init_eng(args, db_frontend )   
               if False == bRet:
                   shutil.move(logname, f"{args.output_dir}/{Engine.LOGFILE}")
                   sys.exit(1)
               #end add
               logging.info("Will generate off-target for functions {}".format(each_line))
               try:
                   engine.generate_off_target([each_line], depth=10000)
               except:
                   logging.info("An exception occurred in generate_off_target function.")
               shutil.move( args.output_dir, os.path.dirname(os.path.abspath(args.output_dir)) + "/" + each_line )
               #engine.init_eng(args, db_frontend )               
        else:   
        ...
        #modified by zenhumany at 2024.04.16
        if os.path.exists( args.output_dir ) and os.path.isdir( args.output_dir ):
            shutil.move(logname, f"{args.output_dir}/{Engine.LOGFILE}")
        else:
            shutil.move(logname, os.path.dirname(os.path.abspath(args.output_dir)) + "/aot_log_" + args.functions_file.split("/")[-1])
        #end modified
        sys.exit(retcode)
